...
// in order to test coordinates computed for some points along the segment with slight offset of 1/3.
...
int main()
{
    const Point first_point  = Point(0, 0);
    const Point second_point = Point(1, 0);

    Segment_coordinates segment_coordinates(first_point, second_point);

    const Point query_points[6] = { Point(Scalar(2) /Scalar(5) , Scalar(1) /Scalar(3)),
                                    Point(1                    , Scalar(-1)/Scalar(3)),
                                    Point(Scalar(7) /Scalar(10), Scalar(1) /Scalar(3)),
                                    Point(Scalar(-3)/Scalar(10), Scalar(-1)/Scalar(3)),
                                    Point(Scalar(6) /Scalar(5) , Scalar(1) /Scalar(3)),
                                    Point(0                    , Scalar(-1)/Scalar(3))
                                  };
    ...
    for(int i = 0; i < 6; ++i) {
        ...
        if( coordinates[count + 0] - expected_coordinates[count + 0] != Scalar(0) ||
            coordinates[count + 1] - expected_coordinates[count + 1] != Scalar(0)  )
        {
            cout << endl << "Segment_coordinates_with_offset_test: FAILED." << endl << endl;
            exit(EXIT_FAILURE);
        }
        count += 2;
    }
    ...
    cout << endl << "Segment_coordinates_with_offset_test: PASSED." << endl << endl;
